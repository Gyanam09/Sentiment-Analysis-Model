...
def test_logging():
    Batch = namedtuple('Batch', ['text_input', 'video_frames', 'audio_features'])
    mock_batch = Batch(text_input={'input_ids': torch.ones(1), 'attention_mask': torch.ones(1)},
                       video_frames = torch.ones(1),
                       audio_features=torch.ones(1)) 
    mock_loader = DataLoader([mock_batch], batch_size=1) 
